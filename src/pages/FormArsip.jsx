...
export default function FormArsip() {
  ...
  // --- 🔥 STYLE UNTUK CROP WARNA INDIGO (BIRU) & ADAPTIVE HANDLES ---
  ...
  // 🔥 FIX: ADAPTIVE FITTING (PORTRAIT/LANDSCAPE) + MAXIMUM PRESENTATION
  const onImageLoad = (e) => {
    const { width, height } = e.currentTarget;
    
    // Calculate the proportional window to fit the image large while maintaining centering
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setForm(initialForm); setPreview(null); setEnhanced(null); setTimeLeft(null); setOcrDebug(null);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 mt-6 font-sans text-slate-700 pb-20">
      ...
      {/* --- MODAL CROP (MAXIMIZED & COMPACT) --- */}
      {showModal && (
        <div className="fixed inset-0 z-[100] bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] shadow-2xl flex flex-col w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-5 border-b flex justify-between items-center bg-white flex-shrink-0">
              <h3 className="font-bold text-slate-800 text-xs uppercase tracking-widest leading-none">Sesuaikan Area Dokumen</h3>
              <button onClick={() => setShowModal(false)} className="text-slate-400 hover:text-red-500 text-2xl transition-colors">×</button>
            </div>
            
            {/* 🔥 Area Gambar: Maximized, Object-Cover within flexible container */}
            <div className="flex-1 bg-slate-100 flex justify-center items-center overflow-hidden p-2">
            ...
            <div className="p-5 bg-white border-t flex gap-3 flex-shrink-0">
              ...
              <button onClick={executeCropAndUpload} className="flex-[2] bg-indigo-600 text-white py-3 rounded-xl text-[10px] font-bold shadow-lg shadow-indigo-100 active:scale-95 transition-all uppercase leading-none">Potong & Scan Sekarang</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-6 mt-6 rounded-[2rem] border border-slate-50 shadow-sm gap-4">
        <div className="text-left w-full md:w-auto">
          ...
          <p className="text-xs text-slate-400 font-medium mt-1 leading-none">Gunakan pemindaian otomatis untuk efisiensi input data</p>
        </div>
        <div className="flex items-center gap-3 w-full md:w-auto justify-end">
          <button type="button" onClick={() => navigate(-1)} className="bg-slate-100 text-slate-500 hover:bg-slate-200 px-6 py-2 rounded-xl text-xs font-bold transition-all h-9 leading-none border border-slate-200/50">Kembali</button>
        </div>
      </div>
            ...
            <div className="sticky top-6 animate-in fade-in zoom-in-95 duration-300">
              <div className="bg-white border border-slate-100 rounded-[2rem] overflow-hidden shadow-sm p-2 text-center">
                <div className="relative rounded-[1.8rem] overflow-hidden bg-slate-50">
                  ...
                  {loading && <div className="absolute inset-0 bg-black/50 flex items-center justify-center text-white text-sm font-bold animate-pulse">Memproses OCR...</div>}
                  {timeLeft !== null && !loading && (
                    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-slate-900/80 backdrop-blur-md text-white px-5 py-2 rounded-full text-[11px] font-bold flex items-center gap-3 shadow-2xl border border-white/10 whitespace-nowrap">
                      <span className="w-2 h-2 bg-red-500 rounded-full animate-pulse"></span>AUTO DELETE: <span className="font-black text-red-400">{formatTime(timeLeft)}</span>
                    </div>
                  )}
                </div>
                <button type="button" onClick={handleExpire} className="w-full mt-2 py-4 text-[10px] font-bold text-slate-400 hover:text-red-600 uppercase tracking-widest transition-all rounded-xl outline-none leading-none">× Batalkan & Hapus Cloudinary</button>
              </div>
            </div>
          )}
        </div>
          ...
          <form onSubmit={handleSubmit} className="bg-white border border-slate-100 rounded-[2rem] shadow-sm p-8 space-y-8 text-left relative">
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <Input label="Status" name="tingkat_pengembangan" value={form.tingkat_pengembangan} onChange={handleChange} error={errors.tingkat_pengembangan} />
              </div>
            ...
            <button type="submit" disabled={isSaving || loading} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-5 rounded-2xl font-bold uppercase tracking-[0.3em] text-[11px] shadow-lg shadow-indigo-100 transition-all active:scale-95 disabled:opacity-50 outline-none leading-none">Simpan Arsip Digital</button>
          </form>
        </div>
      </div>
    </div>
