...
export default function FormArsip() {
  ...
  const handleExpire = async () => {
    ...
    if (urlToDelete) {
      try {
        ...
        setAlert({ show: true, message: "Sesi habis.", type: "hapus" });
  ...
  // 🔥 BIAR CROP NYESUAIN UKURAN TAPI GAK GEDE BANGET
  const onImageLoad = (e) => {
  ...
  const handleUpload = async (image) => {
    ...
    setAlert({ show: true, message: "OCR Scanning...", type: "info" });
    try {
      ...
      if (res.data.success) {
        ...
        setAlert({ show: true, message: "Berhasil!", type: "update" });
        setTimeLeft(180);
      }
    } catch (e) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        setAlert({ show: true, message: "Disimpan!", type: "simpan" });
  ...
  return (
    <div className="space-y-6 mt-6 pb-20 font-sans text-slate-700 animate-in fade-in duration-500">
      ...
      {/* --- MODAL CROP (COMPACT & SLIM) --- */}
      {showModal && (
        <div className="fixed inset-0 z-[100] bg-slate-900/90 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] shadow-2xl flex flex-col w-full max-w-md overflow-hidden animate-in zoom-in-95">
            ...
            <div className="p-4 bg-slate-100 flex justify-center">
              <div className="max-h-[40vh] overflow-auto rounded-xl">
                <ReactCrop 
                  crop={crop} 
            ...
            <div className="p-4 bg-white border-t flex gap-3">
              <button onClick={() => setShowModal(false)} className="flex-1 py-3 text-[10px] font-bold text-slate-400">BATAL</button>
              <button onClick={executeCropAndUpload} className="flex-[2] bg-indigo-600 text-white py-3 rounded-xl text-[10px] font-bold shadow-lg active:scale-95 transition-all">POTONG & SCAN</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex justify-between items-center bg-white p-6 rounded-[2rem] border border-slate-50 shadow-sm">
        <div className="text-left">
          ...
          <p className="text-xs text-slate-400 mt-1">Digitalisasi Dokumen</p>
        </div>
                ...
                <div className="relative rounded-[1.8rem] overflow-hidden">
                  ...
                  {loading && (
                    <div className="absolute inset-0 bg-white/60 backdrop-blur-md flex items-center justify-center">
                      <div className="text-[10px] font-black text-indigo-600 animate-pulse tracking-widest">ANALYZING...</div>
                    </div>
                  )}
                </div>
        ...
        <div className="lg:col-span-7">
          <form onSubmit={handleSubmit} className="bg-white border border-slate-100 rounded-[2rem] shadow-sm p-8 space-y-8">
            <div className="space-y-4">
              ...
              <Input label="Unit" name="unit_pencipta" value={form.unit_pencipta} onChange={handleChange} />
              <Textarea label="Keperluan" name="keperluan" value={form.keperluan} onChange={handleChange} />
              <Input label="Nominal" name="nominal" value={form.nominal} onChange={handleChange} />
            </div>

            <div className="space-y-4 pt-6 border-t border-slate-50">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <Input label="Box Smt" name="no_box_sementara" value={form.no_box_sementara} onChange={handleChange} />
                <Input label="Box Perm" name="no_box_permanen" value={form.no_box_permanen} onChange={handleChange} />
...
const Input = ({ label, value, error, ...props }) => (
  <div className="flex flex-col gap-1">
    ...
    {error && <span className="text-[8px] text-red-500 font-bold ml-1">{error[0]}</span>}
  </div>
);
...
const Select = ({ label, options, value, ...props }) => (
  <div className="flex flex-col gap-1">
    <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1">{label}</label>
    <select value={value ?? ""} {...props} className="w-full bg-slate-50/50 border border-slate-100 rounded-xl px-4 py-3 text-sm focus:bg-white outline-none">
...
const Textarea = ({ label, value, ...props }) => (
  <div className="flex flex-col gap-1">
    <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1">{label}</label>
    <textarea value={value ?? ""} {...props} className="w-full bg-slate-50/50 border border-slate-100 rounded-xl px-4 py-3 text-sm min-h-[80px] resize-none outline-none" />
  </div>
);